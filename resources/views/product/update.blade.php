...
<div class="container">
    ...
    <div class="row">
        <form action="{{route('product.modify')}}" method="post">
            @csrf
            @method('PUT')
            <input name="id" type="hidden" value="{{$product->id}}">
            <div class="form-group col-md-12">
                <label for="inputEmail4">Nombre</label>
                <input value="{{$product->name}}" name="name" type="text" class="form-control" id="inputEmail4" placeholder="Nombre">
            </div>


            <div class="form-group col-md-12">
                <label for="inputEmail4">Stock</label>
                <input value="{{$product->stock}}" name="stock" type="number" class="form-control" id="inputEmail4" placeholder="Stock">
            </div>


            <div class="form-group col-md-12">
                <label for="inputEmail4">Precio</label>
                <input value="{{$product->price}}" name="price" type="number" class="form-control" id="inputEmail4" placeholder="Precio">
            </div>
            ...
            <div class="form-group col-md-12">
                ...
                <select name="local_id" class="form-control">
                    @foreach($locals as $local)
                        <option value="{{$local->id}}" {{$product->local_id == $local->id ? 'selected' : ''}}>{{$local->name}}</option>
                    @endforeach
                </select>
            </div>
